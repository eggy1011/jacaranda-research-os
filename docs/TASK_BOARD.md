...
|---|---|---|---|
| 0 | Repo sync, doc de-staling, solo workflow, coverage gate 90% | — | In progress |
| 1 | Real A-share evidence: `e2e/`→`pipeline/`, live AKShare client, `jacaranda-real-e2e --symbol … --resume`, model rotation (D-008) | 1 真实A股证据 | Pending |
| 2 | Persistence + jobs: SQLAlchemy/Alembic, arq worker with per-stage resume, projects/runs/packages/artifacts API | 2 文件与项目保存 | Pending |
| 3 | Uploads + parsing: pypdf/python-docx/openpyxl with `upload://{file_id}#page=N` locators into S1 | 2 文件与项目保存 | Pending |
