...
def check_card_series() -> None:
    # Delegate to the shipping runtime validator so the contract check, the renderer and the
    # pipeline all enforce ONE rule set. The card series is validated against the Chinese
    # monolingual package it is compiled from.
    sys.path.insert(0, str(ROOT / "packages"))
    from presentation.cards.validate import validate_series

    series = load(SCHEMA_EXAMPLES / "example-social-card-series.zh-CN.json")
    zh_pkg = load(SCHEMA_EXAMPLES / "example-research-package.zh.json")
    for problem in validate_series(series, zh_pkg):
        check(False, "schema_validation_failed", f"social-card/{problem[:60]}", True, "S8",
              problem)
    check(True, "ok", "social-card/validate_series-invoked", True, "S8")
